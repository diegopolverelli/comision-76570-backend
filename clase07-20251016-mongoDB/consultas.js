db.alumnos.insertMany([
    {
        email:'carlos@test.com', apellido:'Tapia'
    },
    {
        domicilio:'Laprida 2014'
    },
    {
        edad:24, email:"margarita@test.com"
    },
    {
...
db.alumnos.dropCollection()

db.cursos.insertMany([{nombre:"Backend I"}, {nombre:"JS II", horas: 3}])
db.cursos.insertOne({nombre:"SQL"})
...
db.heroes.find({publisher:"DC", id:5})
...
db.heroes.find(
    {
        id:{$gt:4}, 
        publisher:{$ne:"DC"}, 
        name:{$in:["Hulk", "Deadpool", "Robin", "Doctor Strange"]}
    }
)
...
db.alumnos.insertMany([
    ...
    {
        nombre:'Romina', apellido:'Gaitan', email:"rg@test.com"
    },
    {
        nombre:'Cristina', apellido:'Vallejos'
    },
    {
        nombre:'Jimena', apellido:'Rodriguez', email:"jr@test.com"
    },
    {
        nombre:'Cristina II',
    },
...
db.heroes.insertOne({
    id:30, name:"Gamora", powers: ["Strategic mind"]
})
...
// select * from heres2 where name like '%man%';

// proyecciones, sort, skip, limit

db.heroes.find({}, {name:1, team:1, id:1})
db.heroes.find({id:{$gt:7}}, {name:1, team:1, id:1})
db.heroes.findOne({id:{$gt:7}}, {name:1, team:1, id:1})

db.heroes.find({}, {powers:0, alias:0, enemies:0})
// db.heroes.find({}, {powers:0, name:1, alias:0, enemies:0})
db.heroes.find({}, {powers:1, name:1, alias:0, enemies:1})

db.heroes.find({}, {name:1, team:1, id:1, _id:0 })

db.heroes.find({}, {powers:0, alias:0, enemies:0, _id:0})


db.heroes.find({}, {name:1, team:1, id:1, _id:0 }).sort({name:-1})
db.heroes.find({id:{$gte:4}}, {name:1, team:1, id:1, _id:0 }).sort({team:1, name: -1})



db.heroes.find({id:{$gte:4}}, {name:1, team:1, id:1, _id:0 }).sort({team:1, name: -1}).limit(3)
db.heroes.find({id:{$gte:4}}, {name:1, team:1, id:1, _id:0 }).limit(3).sort({team:1, name: -1})


db.heroes.find({id:{$gte:4}}, {name:1, team:1, id:1, _id:0 }).sort({team:1, name: -1}).limit(5).skip(4)

db.heroes.find().limit(3)
db.heroes.find().skip(15)
...
db.heroes.updateMany({publisher:"DC"}, {$inc:{id: 1000}})

db.heroes.updateOne({name:"Hulk"}, {$set:{edad: 42}})
db.heroes.updateOne({name:"Hulk"}, {$rename:{edad: "age"}})

db.heroes.updateOne({name:"Hulk"}, {$max:{age: 30}})

db.heroes.updateOne({name:"Robin"}, {$max:{age: 30}})
db.heroes.updateOne({name:"Robin"}, {$max:{age: 30}} , {upsert:true})



// delete
db.heroes.deleteOne({name:"Batman"})
db.heroes.deleteOne({Publisher:"DC"})
