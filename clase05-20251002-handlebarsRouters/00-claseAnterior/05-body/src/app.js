...
app.post("/api/usuarios", async(req, res)=>{
    ...
    // validaciones van siempre en el controller / o handler
    // NUNCA EN EL MANAGER...!!

    try {
        let nuevoUsuario=await UsersManager.addUser(nombre, email)

        res.setHeader('Content-Type','application/json');
        return res.status(201).json({payload:`Usuario ${nombre} creado...!!!`, nuevoUsuario});
    } catch (error) {
        res.setHeader('Content-Type','application/json');
        return res.status(500).json({error:`Internal server error`})
    }
